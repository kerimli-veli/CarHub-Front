...
const AuctionParticipants = ({ messages }) => {
  ...
  return (
    <div
      ref={containerRef}
      className="custom-scroll overflow-y-auto h-[40vh] sm:h-[330px] px-2"
    >
      {messages.length > 0 && (
        <ul className="space-y-2">
          {messages.map((msg, index) => {
            ...
            return (
              <li
                key={index}
                className={`flex items-start text-xs sm:text-sm px-2 sm:px-3 py-2 rounded-md shadow-sm ${bgColor} ${textColor} animate-fade-in`}
              >
