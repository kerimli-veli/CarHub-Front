...
const ProductDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchProductById = async () => {
      try {
        const response = await fetch(`https://carhubwebappp-c3f2fwgtfaf4bygr.italynorth-01.azurewebsites.net/api/Product/Get/${id}`);
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch("https://carhubwebappp-c3f2fwgtfaf4bygr.italynorth-01.azurewebsites.net/api/Product/GetAll");
        const data = await response.json();
        setAllProducts(data);
      } catch (error) {
