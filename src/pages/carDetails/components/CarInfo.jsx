...
const CarInfo = () => {
  ...
  useEffect(() => {
    if (carId) {
      axios.get(`https://carhubwebappp-c3f2fwgtfaf4bygr.italynorth-01.azurewebsites.net/api/Car/GetById?Id=${carId}`)
      .then((res) => {
        const carData = res.data.data;
        setCar(carData);
      
        const baseImageUrl = "https://carhubwebappp-c3f2fwgtfaf4bygr.italynorth-01.azurewebsites.net/"; // backend server public URL
...
setOtherImages([
  ...
  { imagePath: `${baseImageUrl}${carData.carImagePaths[0]?.salonImage}` },
        ...
        if (userId) {
          axios.get(`https://carhubwebappp-c3f2fwgtfaf4bygr.italynorth-01.azurewebsites.net/api/User/GetById?Id=${userId}`)
